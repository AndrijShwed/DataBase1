        ...
        private void buttonЗберегти_В_Таблицю_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                try
                {
                    ...
                    string lastname = Convert.ToString(this.dataGridViewДодати.Rows[current].Cells[0].Value).Replace("'", "`");
                    string name = Convert.ToString(this.dataGridViewДодати.Rows[current].Cells[1].Value).Replace("'", "`");
                    string surname = Convert.ToString(this.dataGridViewДодати.Rows[current].Cells[2].Value).Replace("'", "`");
                    ...
                    if (lastname != "" && name != "" && surname != "" && village != "")
                    {
                        ...
                        else
                        {
                            ...
                            _command.Parameters.Add("@lastname", MySqlDbType.VarChar).Value = this.dataGridViewДодати.Rows[current].Cells[0].Value.ToString().Replace("'", "`");
                            _command.Parameters.Add("@name", MySqlDbType.VarChar).Value = this.dataGridViewДодати.Rows[current].Cells[1].Value.ToString().Replace("'", "`");
                            _command.Parameters.Add("@surname", MySqlDbType.VarChar).Value = this.dataGridViewДодати.Rows[current].Cells[2].Value.ToString().Replace("'", "`");
